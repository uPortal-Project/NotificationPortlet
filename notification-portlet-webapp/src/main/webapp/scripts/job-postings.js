...
function today() {
  let lenCheck = function(num) {
    return num.toString().length > 1 ? num : '0' + num;
  };
  let d = new Date();

  return (
    lenCheck(d.getMonth() + 1) +
    '/' +
    lenCheck(d.getDate()) +
    '/' +
    d.getFullYear()
...
let jobPostings = (function() {
  ...
  let getJobs = function() {
    let jqxhr = $.getJSON(settings.urls.getNotificationsUrl, function(data) {
  ...
  var populateHiringOffices = function() {
    let offices = _.uniq(_.pluck(jobList, 'source'));
    let sortingOffices = [];
    ...
    let tmpl = _.template(
      $('#' + settings.portletId + 'tmpl_hiringCenters').html(),
      {offices: offices}
    );
  ...
  var displayCategories = function() {
    let tmpl = _.template(
      $('#' + settings.portletId + 'tmpl_categories').html(),
  ...
  let displayJob = function(row) {
    let data = oTable.fnGetData(row);
    data.emailFriend = settings.emailFriend;

    populateTemplate(data);
  };

  let toggleFavorite = function(el) {
    let star = $(el).children('i');

    let jqxhr = $.get(el.href, function() {
      star.toggleClass('fa-star-o fa-star');
    }).fail(function() {
      // console.log("error");
    });

    let aPos = oTable.fnGetPosition(el.parentNode);
    let aData = oTable.fnGetData(aPos[0]);
  ...
  var populateTemplate = function(data) {
    let tmpl = _.template(
      $('#' + settings.portletId + 'tmpl_jobDescriptionModal').html(),
      data
    );
    $('#' + settings.portletId + ' .jobDetailsModal')
      .find('.modal-content')
      .html(tmpl);
    ...
    $('#' + settings.portletId + ' .sendEmailButton').on('click', function(e) {
      $.post(
        'test.php',
        $('#' + settings.portletId + ' .emailFriendForm').serialize()
      );
      $('#' + settings.portletId + ' .modal-overlay').hide();
      toggleModal();
    });
  ...
  let applyFilter = function(def, col) {
  ...
  var initTable = function() {
    ...
    $.fn.dataTableExt.afnFiltering.push(function(oSettings, aData, iDataIndex) {
      let min = document.getElementById(settings.portletId + 'date-range')
        .value;
      if (min === '') {
        return true;
      }
      ...
      let colDate = Date.parse(aData[iStartDateCol]);
    ...
    $.extend(true, $.fn.dataTable.defaults, {
      sDom:
        '<\'row\'<\'col-md-6 toggleCheckbox\'><\'col-md-6 text-right\'l>t<\'row\'<\'col-md-6\'i><\'col-md-6\'p>>',
    ...
    $.extend($.fn.dataTableExt.oPagination, {
      bootstrap: {
        fnInit: function(oSettings, nPaging, fnDraw) {
          ...
          $(nPaging).append(
            '<ul class="pagination">' +
              '<li class="prev disabled"><a href="#"><i class="fa fa-caret-left"></i> ' +
              oLang.sPrevious +
              '</a></li>' +
    ...
    $('#' + settings.portletId + 'jobPostings').dataTable({
      ...
      aoColumnDefs: [
          ...
          mRender: function(data, type, full) {
            if (data[0].toLowerCase() === 'open') {
              return (
                '<a href="' +
                full.url +
                '" target="_blank" class="btn btn-sm btn-success">Apply</a>'
              );
            } else {
        ...
        },
        {
          ...
          mRender: function(data, type, full) {
            let favClass, linkTitle;
            if (full.attributes.status[0].toLowerCase() === 'open') {
              let url = settings.urls.invokeActionUrlTemplate
                .replace(/NOTIFICATIONID/, full.id)
                .replace(/ACTIONID/, 'FavoriteAction');
              if (data === true) {
                return (
                  '<span class="sr-only">' +
                  data +
                  '</span><a href="' +
                  url +
                  '" title="Remove from saved jobs"><i class="fa fa-star"></i></a>'
                );
              } else {
                return (
                  '<span class="sr-only">' +
                  ...
                  '</span><a href="' +
                  url +
                  '" title="Add to saved jobs"><i class="fa fa-star-o"></i></a>'
                );
              }
            } else {
              return '';
      ...
      fnInitComplete: function(oSettings, json) {
        ...
        $('#' + settings.portletId + 'jobPostings').delegate(
          'td.favorite a',
          'click',
          function(e) {
            e.preventDefault();
        ...
        // cbl.html(tcb);
        $('.toggleCheckbox').append(
          cbl.html(tcb).append('Show "apply in person" jobs')
        );
